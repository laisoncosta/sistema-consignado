...
## Cerca virtual (GPS)
Validação no envio só quando **triplo check** ativo:
1. Promotor com `cercaVirtualAtiva` no cadastro
2. Loja com cerca ativa
3. Loja com perímetro > 0 e coordenadas configuradas

Comportamento:
- Triplo check desativado → pedido segue sem validação GPS
- GPS indisponível → bloqueado com orientação para ativar localização
- Loja sem coordenadas → bloqueado — contatar administrador
- Fora do perímetro → bloqueado com distância exibida
- Dentro do perímetro → permitido; `distanciaLojaMetros` gravada no pedido
- Admin/Diretor no modo gestão do portal: sem bloqueio de cerca
...
- `CHECKIN_GPS_OBRIGATORIO = false`: promotores CLT podem lançar sem check-in obrigatório
- Em dev com túnel HTTPS, cerca pode ser ignorada no servidor
...
| Cerca virtual (triplo check) | Sim | `02-Regras-Negocios.md` §7, `lib/cerca-virtual.ts` |
