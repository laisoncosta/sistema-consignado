...
export async function PUT(request: Request, context: RouteContext) {
  ...
  const transferenciaId = Number(idParam);

  if (!Number.isInteger(transferenciaId) || transferenciaId <= 0) {
  ...
  try {
    ...
    const existente = await prisma.transferenciaAvulsa.findFirst({
      where: {
        id: transferenciaId,
        loja: { ativo: true, ...filtroLojaRegiao },
      },
    });
    ...
    const transferencia = await prisma.transferenciaAvulsa.update({
    ...
    console.error("Erro ao atualizar transferência avulsa:", error);
    return NextResponse.json(
