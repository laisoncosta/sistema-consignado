...
type ConferenciaLancamentosModalProps = {
  ...
  inline?: boolean;
  /** Conferência do pedido extra: só produto + quantidade digitada (> 0). */
  modoPedidoExtra?: boolean;
};
...
function linhaPossuiPedidoExtra(linha: CamposLinhaPedido): boolean {
  return Number(linha.pedido || 0) > 0;
}
...
export function ConferenciaLancamentosModal({
  ...
  const produtosComLancamento = produtos.filter((produto) => {
    ...
    return modoPedidoExtra
      ? linhaPossuiPedidoExtra(linha)
      : linhaPossuiQuantidade(linha);
  });
    ...
    >
      <div className="shrink-0 border-b border-slate-200 px-5 py-4 sm:px-6">
        ...
        <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900">
          {modoPedidoExtra
            ? "Confira as quantidades do pedido extra antes de enviar. Apenas produtos com quantidade acima de zero são listados."
            : AVISO_CONFERENCIA}
        </p>
            ...
            <table className="w-full min-w-[280px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  {modoPedidoExtra ? (
                    <th className="px-3 py-2.5 text-center">Qtde</th>
                  ) : (
                    <>
              ...
              <tfoot>
                <tr className="border-t-2 border-slate-300 bg-slate-200">
                  <td className="px-3 py-2.5 text-xs font-bold uppercase tracking-wide text-slate-800">
                    Totais
                  </td>
                  {modoPedidoExtra ? (
