...
function ResumoPeriodoMobile({
  ...
  const camposResumo = [
    ...
    { label: "Qtde Traf Avulsa", valor: qtdeTransf },
  ];
    ...
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        {camposResumo.map((campo) => (
          ...
          >
            <p className="font-medium text-slate-500">{campo.label}</p>
      ...
      >
        <p className="text-xs font-medium text-slate-500">Pedido Total</p>
...
function ValorPedidoTotalHistorico({
  ...
  if (status === "Aprovado" || status === "Aprovado Parcialmente") {
    return (
      <p className={`${className} text-slate-900`}>
        <CelulaNumerica valor={pedidoTotal} />
      </p>
    );
  }
  ...
  if (status === "Reprovado") {
    return (
      <p className={`${className} text-rose-700`}>Reprovado</p>
    );
  }

  return (
    <p className={`${className} text-amber-900`}>Aguardando Aprovação</p>
  );
}
...
export function HistoricoPedidos({
  ...
  const classeSelectStatus = otimizadoMobile
    ? portalSelectMobile.replace("pl-10", "px-2")
  ...
  const classeSelectCompactoMobile =
    "min-h-11 w-full appearance-none rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs text-slate-800 outline-none transition focus:border-transparent focus:ring-2 sm:text-sm";
  const classeData = otimizadoMobile
    ? "min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 py-3.5 text-base text-slate-800 outline-none transition focus:border-transparent focus:ring-2 sm:min-h-11 sm:py-3 sm:text-sm"
      ...
      <div className="mt-5 space-y-3 md:hidden">
        ...
        quantidadeProdutosPendentes > 0 ? (
          <button
            ...
            className="w-full rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-left transition active:bg-amber-100"
          >
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-slate-600">{linha.loja}</p>
                  </div>
                ...
                {linha.pedidoExtraSolicitado > 0 ? (
                  <div className="mt-4 rounded-xl border-2 border-green-500 bg-green-50/20 p-4">
                ...
                {linha.qtdeTransf > 0 || linha.bonificacao > 0 ? (
                  <div className="mt-4 rounded-xl border-2 border-amber-600 bg-amber-50/20 p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-700">
                      Transferência Avulsa
                    </p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                        <p className="font-medium text-slate-500">Qtde Transf</p>
                        <p className="mt-1 text-base font-semibold text-slate-900">
                          <CelulaNumerica valor={linha.qtdeTransf} />
                        </p>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                        <p className="font-medium text-slate-500">Bonificação</p>
                        <p className="mt-1 text-base font-semibold text-slate-900">
                    ...
                    {linha.motivo.trim() ? (
                      <div className="mt-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs">
                        <p className="font-medium text-slate-500">Motivo</p>
                ...
                <div
                  ...
                  style={{ backgroundColor: "#F1F5F9" }}
                >
                  <p className="text-xs font-medium text-slate-600">Pedido Total</p>
